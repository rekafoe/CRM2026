...
| **Обложка** (`cover.mode = separate`) | Строка **«Обложка (печать и материал)»** в `services`; материал обложки — в шаблоне вкладка **«Сборка»** → `cover.material_id` |
...
**Обложка с другим материалом:** в шаблоне **Сборка** → режим обложки **«Отдельная»** (`cover.mode = separate`), поле **материал обложки** и при необходимости отдельные параметры печати обложки. В калькуляторе клиент выбирает материал **блока**; обложка берётся из шаблона (или `cover_material_id` в конфиге, если позже добавят в UI).
