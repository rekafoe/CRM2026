...
### Страницы в калькуляторе = всего в альбоме (с обложкой)

Если в шаблоне (**Сборка → Обложка**) режим не «Без», поле **«Страницы»** в калькуляторе — это **полное число страниц готового альбома**, обложка уже внутри.

- В **Сборке** задаётся **«Страниц обложки»** (по умолчанию 4) — они вычитаются из общего числа.
- **Блок** = `страницы − страниц_обложки` (например 28 − 4 = **24**).
- **Переплёт** и лимиты варианта bind проверяются по **полным** 28 стр.

| Режим обложки | Печать и материал блока | Обложка в цене |
|---------------|-------------------------|----------------|
| **Без** | все страницы | — |
| **Та же бумага, что блок** (`self`) | все 28 стр. одним контуром | не добавляется отдельной строкой |
| **Своя бумага** (`separate`) | только страницы блока (24) | отдельно по 4 стр. обложки и своему материалу |

**Тиражная скидка на печать:** для выбора диапазона в `print_prices.tiers` используется **суммарный** объём печати **блок + обложка** (`multipagePrintUnits + coverPrintUnits`), как один прогон на станке. Стоимость по-прежнему: `unit_price × объём_блока` и `unit_price × объём_обложки` по **одной** ставке tier от суммы.

### Как считается «объём печати»

Только для продуктов с **product_type = 'multi_page'**:

1. **Листов на одно изделие** (sheetsPerItem) — по **раскладке** формата на печатный лист (`itemsPerSheet` с бэкенда), по **страницам блока** (или по всем страницам при `self`):
...
- **Печать**  
  В шаблоне у размера (обычно **A4**) в **print_prices** хранится **unit_price за одно поле раскладки** (при «Заполнить из центра» это `цена_за_физический_лист_SRA3 / itemsPerSheet`).  
  Физических листов на альбом: `effectivePrintQuantity` (см. формулу выше).  
  Позиций печати: `multipagePrintUnits = effectivePrintQuantity × itemsPerSheet` (например 7 листов × 2 поля A4 = **14**).  
  Диапазон tier выбирается по **multipagePrintUnits** (блок), при отдельной обложке — по **multipagePrintUnits + coverPrintUnits**.  
  **Стоимость печати блока = unit_price × multipagePrintUnits**; обложка (`separate`) — **unit_price × coverPrintUnits** с тем же tier.
...
### Таблица в заказе (печать / резка / переплёт)

Позиция заказа показывает строки из **`params.services`** и **`params.materials`** (см. `OrderItemPositionBreakdown`).

| Компонент расчёта | Откуда в заказе |
|------------------|-----------------|
| Блок (печать + бумага блока) | Строка **«Печать …»** (+ бумага блока, свёрнута в подпись печати) |
| Резка по раскладке | Услуга **«Резка»** в `services` (из `finishing` / авто-резка) |
| **Переплёт** из `multiPageStructure.binding` | Услуга **«Переплёт …»** в `services` (`operation_type = bind`), цена из `BindingPricingService` |
| **Обложка** (`cover.mode = separate`) | Строка **«Обложка (печать и материал)»** в `services`; бумага — `cover_material_id` в заказе / расчёте |
| Итого | `finalPrice` = блок + обложка + переплёт + прочая отделка |

**Материал переплёта:** не выбирается в калькуляторе как бумага. Расходники переплёта (пружина, клеевой слой и т.д.) задаются в карточке **услуги переплёта** / **варианта** (`service_variants.material_id`, `qty_per_item`) — при наличии попадут в `operationMaterials` для склада.

**Обложка с другим материалом:** в шаблоне **Сборка** → режим **«Отдельная»** (`cover.mode = separate`) → галочки **разрешённых материалов обложки** (`cover.allowed_material_ids`) и **материал по умолчанию** (`cover.material_id`). В калькуляторе CRM — блок **«Обложка»**: выбор бумаги из разрешённого списка (`specifications.cover_material_id`). Печать обложки по-прежнему из шаблона (`cover.print`).
