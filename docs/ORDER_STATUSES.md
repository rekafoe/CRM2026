...
**Счётчики / выручка за день:** в кассу не попадают заказы со `status=0` (пул «Ожидает»). Заказы со `status=1` («Оформлен») учитываются, если есть оплаченная предоплата (`prepaymentStatus` paid/successful, или офлайн с суммой). Сумма берётся из `cash_for_report_date` (день оплаты / выдачи, не день работы без оплаты).
