...
- **Multi-size ZIP:** папки размеров в корне архива (`204x204/`, `148х210/` — латиница/кириллица `x`/`х`). В каждой папке — SVG-страницы **одного** физического формата. Импорт создаёт один `design_code` и N строк `design_templates`. Источник мм — SVG; имя папки — подсказка (при расхождении > 1,5 мм — warning).
