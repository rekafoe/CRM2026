...
- Принимается только **SVG** (контракт по именам в `id`, `inkscape:label` или `data-name`).
...
- `preview_url` указывает на **SVG после вырезания** указанных слоёв (исходный файл не подменяет превью);
- `spec.width_mm`, `spec.height_mm`, `spec.page_count`;
- `spec.import` содержит метаданные импорта: `importerVersion: 3`, `strippedInteractiveLayers`, распознанные `guideRectsMmParsed`, предупреждения и ошибки;
- `spec.designState` содержит страницу Fabric (фото- и текстовые поля), при наличии направляющих — **`prepress`**, а фон в редакторе подставляется с превью.
