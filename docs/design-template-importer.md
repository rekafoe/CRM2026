...
| `decor_*` | Интерактивный декоративный объект. На текущем этапе поддержаны SVG `rect`, `circle`, `path`. |
...
- Объекты без префикса (`photo_*`, `text_*`, `decor_*`) не импортируются интерактивно. Если при этом отсутствует `locked_bg`, importer добавляет warning с подсказкой переименовать слой в `decor_*` или добавить `locked_bg`.
