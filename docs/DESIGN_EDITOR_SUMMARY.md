...
- Правая панель клиентского редактора: фото (загрузка, библиотека, постановка в поля) и текстовые поля текущего фрагмента (preflight, фокус на canvas). На мобиле — вкладки dock «Макет / Фото / Текст / Проверка». Вкладка «Проверка» (десктоп — панель задач внизу сайдбара): список ошибок preflight, переход на разворот по клику. Модалка перед заказом: миниатюры по полосе страниц/разворотов (как в навигаторе), для разворотов — две страницы рядом.
...
- Нормализация нечётного числа внутренних страниц при открытии multipage в режиме разворотов (без лишнего dirty); добавление разворота всегда +2 страницы.
...
| Инструменты | + Текст / + Заголовок / фото-поле / фигура появляются на canvas; «Слои/объект» → Удалить для добавленного текста/фигуры |
| Mobile | Dock Макет/Фото/Текст/Проверка; на вкладках Фото/Текст canvas скрыт |
