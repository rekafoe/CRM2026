...
- Клиентский текст (`textFieldClientAdded`, basic): блок **Textbox** на ширину safe zone — длинные надписи переносятся по строкам; preflight предупреждает, если текст выходит за trim или safe zone (вкладка «Проверка»). Шаблонный однострочный `i-text` из SVG по-прежнему можно сузить ресайзом или кеглем.
...
| Инструменты | + Текст / + Заголовок / фото-поле (~30% ширины страницы) / фигура; ресайз фото-поля и текстовых блоков углами (кегль запекается в fontSize); «Слои/объект» → Удалить |
