...
Юридический клиент ищется только среди `type=legal` по точному `TRIM(tax_id)`. Телефон и email не используются для связывания, поэтому карточка физлица не будет превращена в юрлицо. Если карточка с таким УНП уже есть, заказ только привязывается к ней — реквизиты (`bank_details`, контакты, уполномоченное лицо и т.п.) из website checkout **не перезаписываются**. Новая legal-карточка создаётся с `source=website` только когда УНП ещё не найден.
