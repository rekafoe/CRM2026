...
- CRM sandbox для проверки клиентского редактора: `/adminpanel/public-design-editor-preview/:templateId`.
...
- Для позиций из редактора — приложить production PDF, собранный постранично в порядке `1..n`.
- Backend-proxy к CRM для endpoints, которые требуют `WEBSITE_ORDER_API_KEY`.
