...
Если бэкенд деплоится на **Railway** и при каждом пуше шаблоны слетают:

1. **Volume уже есть** — проверьте, что задана переменная окружения **`DOCUMENT_TEMPLATES_DIR`** и она указывает **точно на путь монтирования volume** внутри контейнера (например `/data` или `/data/document-templates`).
2. **Путь volume в Railway** — при создании Volume в Railway указывается **Mount Path** (например `/data`). Тогда задайте:  
   **`DOCUMENT_TEMPLATES_DIR=/data/document-templates`**  
   и при первом запуске приложение создаст папку `document-templates` внутри volume (при загрузке шаблона через UI).
3. **Если переменная не задана** — приложение пишет шаблоны в `dist/templates` внутри контейнера; при редеплое контейнер пересоздаётся и файлы пропадают. В логах при старте будет предупреждение: *"DOCUMENT_TEMPLATES_DIR not set"*.
4. **После настройки** один раз загрузите шаблоны через UI или положите файлы с именами из БД в volume (см. раздел 5). Дальнейшие редеплои шаблоны не затронут.
