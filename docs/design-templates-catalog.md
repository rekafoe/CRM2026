...
- **Обновить SVG** (иконка загрузки на карточке) — повторный импорт в **тот же** шаблон: `POST /api/design-templates/:id/reimport`. Меняются `designState`, превью и метаданные импорта; **id**, категория, автор, привязки в `product_subtype_designs` сохраняются. Флаг «активен» не включается автоматически, если шаблон был выключен.
