...
Справочник **`design_template_categories`** (имя, `sort_order`). У шаблона поле **`design_templates.category`** — текст, совпадающий с `name` в справочнике (не FK).

- **Каталог** группирует карточки только по категориям, где есть шаблоны (пустые из справочника не показываются); отдельно **«Без категории»** и устаревшие имена вне справочника. Секции можно сворачивать (состояние в `localStorage`). Бейдж **«нет привязки · настроить»** открывает вкладку «Привязки» (если в spec есть product/type).
- Кнопка **«Категории»** — добавить, переименовать, удалить, изменить порядок. Переименование обновляет `design_templates.category` у всех шаблонов. Удаление снимает категорию.
- Фильтр **«Без привязки»** и бейдж на карточке — шаблон не в матрице `product_subtype_designs` (поле `subtype_link_count` в списке каталога).
- В формах импорта/карточки — выбор из списка или **«Новая»** (создаёт запись в справочнике).
- API (после `authenticate`): `GET/POST /api/design-templates/categories`, `PUT/DELETE /api/design-templates/categories/:id`.

Public API категории не отдаёт.
