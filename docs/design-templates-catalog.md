...
| `usage_fee` (Y) | Плата за использование макета **один раз на позицию** с макетом; на сайте: итог = **X×qty + Y**, где X — цена продукта за ед. |
| `author_percent` (Z) | % автору **от Y**, не от X |

**Сайт:** `priceType` (срочный/промо) применяется только к X; на Y не действует. Y начисляется **один раз на строку** (визитки 24 шт. с одним дизайном → Y×1). Public API отдаёт `design_code` и `usage_fee`, **не** отдаёт `author_user_id` / `author_percent`.

**Начисление ЗП** (`order_item_earnings`):

| Тип | Кому | База |
|-----|------|------|
| `operator` | Исполнитель заказа | Только **X** (из `price×qty` вычитается `usage_fee` / `params.designUsageFee` один раз) |
| `design_author` | `author_user_id` шаблона | **Y** один раз, выплата × Z% |

Пример: Y = 4, Z = 10%, qty = 24 → автору 0,40; оператор % с Y не получает.
