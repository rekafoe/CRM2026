...
- **Точка входа** — основной файл: `backend/src/index.ts` (скрипты `npm run dev` / `npm start` указывают на него).
- **Дублирование сервисов** — актуальный модуль: `backend/src/modules/shared/services/optimizedQueries.ts` (при появлении второй копии свести к одному источнику).
...
| Средний  | Точка входа backend        | `src/index.ts` (готово)                         | Единый entry              |
