  ...
  return (
    <div className="order-item-horizontal">
      ...
      {(() => {
        ...
        // Это предотвращает дублирование, так как оба означают тип бумаги со склада
        if (fromType && fromMaterial && fromType === fromMaterial) {
          // Если "Тип материала" совпадает с "Материал" - не показываем отдельно
          return null;
        }
        
        // 🆕 Приоритет: materialTypeDisplay (display_name из склада) > fromType (из parameterSummary, уже отформатирован) > materialTypeRaw
        // Это гарантирует, что всегда показывается транслированное название типа бумаги со склада
