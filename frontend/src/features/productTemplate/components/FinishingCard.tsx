  ...
  <div className={`simplified-card simplified-card--collapsible ${!expanded ? 'simplified-card--collapsed' : ''}`}>
    ...
    <div className="simplified-card__content">
      {loadingLists ? (
        ...
        return (
          <>
            {servicePricings.length === 0 && firstServiceItem && (
              <Alert type="info" className="mb-2">
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  <span>Услуги отделки ещё не добавлены для этого размера.</span>
                  <Button
                    type="button"
                    variant="secondary"
                    size="sm"
                    onClick={() => {
                      ...
                      updateSize(selected.id, { finishing: [firstServiceItem] as any })
                    }}
                  >
                    Добавить услугу
                  </Button>
