...
export interface PublicEditorPreflightPageAnalysis {
  pageIndex: number;
  photoFields: PublicEditorPreflightField[];
  textFields: PublicEditorPreflightField[];
  issues: PublicEditorPreflightIssue[];
...
export function analyzePublicDesignPage(
  page: DesignPage | undefined,
  pageIndex: number,
  pageBounds?: DesignPageBoundsPx,
): PublicEditorPreflightPageAnalysis {
  ...
  walkFabricObjects(page?.fabricJSON, (obj) => {
    ...
    if (obj.isPhotoField === true) {
      ...
      photoFields.push({
        ...
        detail: filled
          ? lowQuality ? 'Фото может быть низкого качества для печати' : 'Фото добавлено'
          : 'Нужно добавить фото',
      });
      if (!filled) {
        issues.push({
...
export function buildPublicDesignPreflightSummary(
  pageAnalyses: PublicEditorPreflightPageAnalysis[],
  ...
  for (const analysis of pageAnalyses) {
    analysis.photoFields.forEach((field) => {
      photoOrdinal += 1;
      photoFields.push({
        ...field,
        label: `Фото ${photoOrdinal}`,
      });
    });
    textFields.push(...analysis.textFields);
...
export function analyzePublicDesignPages(
  ...
  try {
    const pageAnalyses = pages.map((page, pageIndex) => analyzePublicDesignPage(page, pageIndex, pageBounds));
    return buildPublicDesignPreflightSummary(pageAnalyses);
  } finally {
    stop({ saveState });
  }
}
