...
import { StockMonitoringService } from '../../warehouse/services/stockMonitoringService';
import { AutoOrderService } from '../../telegram/services/autoOrderService';
...
export class NotificationController {
  ...
  static async getTelegramConfig(req: Request, res: Response) {
    try {
      ...
      logger.error('Error getting Telegram config', error);
      res.status(500).json({
        success: false,
  ...
  static async configureTelegram(req: Request, res: Response) {
    try {
      ...
      logger.debug('Configuring Telegram', { 
  ...
  static async getStockAlerts(req: Request, res: Response) {
    logger.debug('getStockAlerts called');
    try {
      logger.debug('Calling MaterialService.getLowStockMaterials()...');
      const materials = await MaterialService.getLowStockMaterials();
      logger.debug(`Got ${materials.length} low stock materials`, { count: materials.length });
  ...
  static async getStockMonitoringConfig(req: Request, res: Response) {
    try {
      ...
      logger.error('Error getting stock monitoring config', error);
      res.status(500).json({
        success: false,
  ...
  static async updateStockMonitoringConfig(req: Request, res: Response) {
    try {
      ...
      logger.error('Error updating stock monitoring config', error);
      res.status(500).json({
        success: false,
  ...
  static async createAutoOrder(req: Request, res: Response) {
    try {
      ...
      logger.error('Error creating auto order', error);
      res.status(500).json({
        success: false,
  ...
  static async getAutoOrders(req: Request, res: Response) {
    try {
      ...
      logger.error('Error getting auto orders', error);
      res.status(500).json({
        success: false,
  ...
  static async approveAutoOrder(req: Request, res: Response) {
    try {
      ...
      logger.error('Error approving auto order', error);
      res.status(500).json({
        success: false,
  ...
  static async sendAutoOrder(req: Request, res: Response) {
    try {
      ...
      logger.error('Error sending auto order', error);
      res.status(500).json({
        success: false,
  ...
  static async markAutoOrderDelivered(req: Request, res: Response) {
    try {
      ...
      logger.error('Error marking auto order as delivered', error);
      res.status(500).json({
        success: false,
  ...
  static async getAutoOrderConfig(req: Request, res: Response) {
    try {
      ...
      logger.error('Error getting auto order config', error);
      res.status(500).json({
        success: false,
        message: 'Ошибка получения конфигурации',
        error: error.message
  ...
  static async updateAutoOrderConfig(req: Request, res: Response) {
    try {
      ...
      logger.error('Error updating auto order config', error);
      res.status(500).json({
        success: false,
        message: 'Ошибка обновления конфигурации',
        error: error.message
