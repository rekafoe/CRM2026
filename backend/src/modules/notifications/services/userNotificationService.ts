...
export class UserNotificationService {
  ...
  static async sendToUser(userId: number, notification: NotificationMessage): Promise<boolean> {
    ...
    if (!user.notifications_enabled) {
      logger.debug(`Notifications disabled for user ${user.name}`);
      return false;
    }
    ...
    if (!preferences[notification.type]) {
      logger.debug(`${notification.type} notifications disabled for user ${user.name}`);
      return false;
    }
    ...
    if (user.telegram_chat_id) {
      ...
      if (success) {
        logger.debug(`Notification sent to user ${user.name} (${user.role})`);
        return true;
      }
    }

    logger.warn(`No Telegram chat_id for user ${user.name}`);
    return false;
  }
  ...
  static async sendToRole(role: string, notification: NotificationMessage): Promise<number> {
    ...
    logger.info(`Sending notification to ${users.length} ${role} users`);
    ...
    logger.info(`Notification sent to ${sentCount}/${users.length} ${role} users`);
    return sentCount;
  }
  ...
  static async sendToAllUsers(notification: NotificationMessage): Promise<number> {
    ...
    logger.info(`Notification sent to ${sentCount}/${users.length} users`);
    return sentCount;
  }
  ...
  static async updateUserTelegramChatId(userId: number, chatId: string): Promise<boolean> {
    ...
    try {
      ...
      logger.debug(`Telegram chat_id update skipped - column doesn't exist for user ${userId}`);
      return true;
    } catch (error) {
      logger.error(`Failed to update Telegram chat_id for user ${userId}`, error);
      return false;
    }
  }
  ...
  static async sendTestMessageToBotUsers(): Promise<number> {
    ...
    logger.info(`Sending test message to ${botUsers.length} bot users`);
    ...
    for (const user of botUsers) {
      try {
        ...
        if (result.ok) {
          sentCount++;
          logger.debug(`Test message sent to ${user.first_name} (@${user.username || 'no_username'})`);
        } else {
    ...
    logger.info(`Test messages sent to ${sentCount}/${botUsers.length} users`);
    return sentCount;
  }
}
