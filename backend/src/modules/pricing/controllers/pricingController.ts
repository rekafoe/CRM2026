...
import { MarkupSettingsService } from '../services/markupSettingsService'
import { QuantityDiscountsService } from '../services/quantityDiscountsService'
...
export class PricingController {
  ...
  static async calculateProductPrice(req: Request, res: Response) {
    const {
      productId,
      quantity,
      qty: qtyParam,
  ...
  static listServices = async (req: Request, res: Response) => {
  ...
  static listServiceVolumePrices = async (req: Request, res: Response) => {
  ...
  static listPrintPrices = async (_req: Request, res: Response) => {
    const prices = await PrintPriceService.list()
    res.json(prices)
  }

  static createPrintPrice = async (req: Request, res: Response) => {
    const {
      technology_code,
      counter_unit,
      price_bw_single,
  ...
  static updatePrintPrice = async (req: Request, res: Response) => {
    const { id } = req.params
  ...
  static deletePrintPrice = async (req: Request, res: Response) => {
    const { id } = req.params
    await PrintPriceService.delete(Number(id))
    res.status(204).end()
  }

  static async createServiceVolumePrice(req: Request, res: Response) {
    const { serviceId } = req.params
  ...
  static updateServiceVolumePrice = async (req: Request, res: Response) => {
  ...
  static deleteServiceVolumePrice = async (req: Request, res: Response) => {
    const { tierId } = req.params
  ...
  private static async ensureOperationNormsTable() {
    ...
    try {
      ...
      const presets: Array<{
        ...
        // Многостраничные продукты (multi_page)
        {
          productType: 'multi_page',
          operationLabel: 'Печать',
          candidates: ['Цифровая цветная печать (SRA3)', 'Цифровая печать', 'Офсетная печать'],
          formula: 'ceil(pages / 2) * sides' // Печать рассчитывается по листам (страницы/2 * стороны)
        },
        {
          productType: 'multi_page',
          operationLabel: 'Резка/Фальцовка',
          candidates: ['Резка на гильотине', 'Фальцовка', 'Резка многостраничных'],
          formula: 'quantity' // Резка рассчитывается по количеству изделий
        },
        {
          productType: 'multi_page',
          operationLabel: 'Переплет',
          candidates: ['Скрепка', 'Клеевое скрепление', 'Твердый переплет', 'Пружина'],
          formula: 'quantity' // Переплет рассчитывается по количеству изделий
        },
        // Универсальные продукты (universal)
        {
          productType: 'universal',
          operationLabel: 'Печать',
          candidates: ['Цифровая цветная печать (SRA3)', 'Цифровая печать', 'Лазерная печать'],
          formula: 'quantity' // Универсальная формула, будет уточняться в зависимости от типа изделия
        },
        {
          productType: 'universal',
          operationLabel: 'Резка/Отделка',
          candidates: ['Резка на гильотине', 'Вырубка', 'Скругление углов', 'Ламинация'],
          formula: 'quantity' // Универсальная формула для отделочных операций
        }
      ]
  ...
  static getProductSchema = asyncHandler(async (req: Request, res: Response) => {
    ...
    const schema = {
      ...
      operations: operations.map(op => ({
        ...
        formula: op.operation === 'Печать' ? 'calculated_by_layout_service' : op.formula,
        note: op.operation === 'Печать' ? 'Количество листов рассчитывается LayoutCalculationService на основе размеров и раскладки' : null
      }))
    }
  ...
  // --- Markup Settings CRUD ---
  static listMarkupSettings = async (_req: Request, res: Response) => {
    const settings = await MarkupSettingsService.list()
    res.json(settings)
  }

  static createMarkupSetting = async (req: Request, res: Response) => {
    const { setting_name, setting_value, description, is_active } = req.body

    if (!setting_name) {
      res.status(400).json({ message: 'setting_name обязателен' })
      return
    }

    const setting = await MarkupSettingsService.create({
      setting_name,
      setting_value: Number(setting_value ?? 0),
  ...
  static updateMarkupSetting = async (req: Request, res: Response) => {
    const { id } = req.params
    const { setting_name, setting_value, description, is_active } = req.body

    const updated = await MarkupSettingsService.update(Number(id), {
      setting_name,
      setting_value: setting_value !== undefined ? Number(setting_value) : undefined,
      description,
      is_active: is_active !== undefined ? !!is_active : undefined,
    })

    if (!updated) {
      res.status(404).json({ message: 'markup setting not found' })
      return
    }
  ...
  static deleteMarkupSetting = async (req: Request, res: Response) => {
    const { id } = req.params
    await MarkupSettingsService.delete(Number(id))
    res.status(204).end()
  }

  // --- Quantity Discounts CRUD ---
  static listQuantityDiscounts = async (_req: Request, res: Response) => {
    const discounts = await QuantityDiscountsService.list()
    res.json(discounts)
  }

  static createQuantityDiscount = async (req: Request, res: Response) => {
    const { min_quantity, max_quantity, discount_percent, description, is_active } = req.body

    if (min_quantity === undefined || discount_percent === undefined) {
      res.status(400).json({ message: 'min_quantity и discount_percent обязательны' })
      return
    }

    const discount = await QuantityDiscountsService.create({
  ...
  static updateQuantityDiscount = async (req: Request, res: Response) => {
    const { id } = req.params
    const { min_quantity, max_quantity, discount_percent, description, is_active } = req.body

    const updated = await QuantityDiscountsService.update(Number(id), {
      min_quantity: min_quantity !== undefined ? Number(min_quantity) : undefined,
      max_quantity: max_quantity !== undefined ? (max_quantity ? Number(max_quantity) : null) : undefined,
    ...
    if (!updated) {
      res.status(404).json({ message: 'quantity discount not found' })
      return
    }
  ...
  static deleteQuantityDiscount = async (req: Request, res: Response) => {
    const { id } = req.params
