...
import { OptimizedQueries } from '../../shared/services/optimizedQueries'
...
export class OptimizedMaterialController {
  ...
  static getMaterials = asyncHandler(async (req: Request, res: Response) => {
    ...
    const cacheKey = `materials:list:${filters.categoryId || 'all'}:${filters.supplierId || 'all'}:${filters.search || ''}`
  ...
  static invalidateCache = asyncHandler(async (req: Request, res: Response) => {
    cacheService.deletePattern('GET:/api/materials')
