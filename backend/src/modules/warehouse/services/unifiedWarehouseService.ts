...
export class UnifiedWarehouseService {
    ...
    try {
      ...
      const materials = await db.all(`
        SELECT 
          ...
          -- Используем sheet_price_single как актуальную цену
  ...
  static async reserveMaterials(reservations: {
    ...
    try {
      ...
      for (const reservation of reservations) {
        ...
        // Проверяем доступное количество (учитываем только активные резервы, которые не истекли)
        const now = new Date().toISOString();
        const existingReservations = await db.get<{ reserved: number }>(`
        ...
        const reserved = existingReservations?.reserved || 0;
      ...
      logger.error('Ошибка резервирования материалов', { error, reservations });
      throw error;
    }
  }
    ...
    try {
      ...
      for (const reservationId of reservationIds) {
        ...
        await MaterialTransactionService.spend({
          ...
          userId: undefined // user_id можно получить из контекста
        });
  ...
  static async syncWithPricing(): Promise<void> {
    try {
      const materials = await this.getAllMaterials();
      logger.info('Цены материалов синхронизированы', { count: materials.length });
    } catch (error) {
      logger.error('Ошибка синхронизации с системой ценообразования', error);
    }
  }
}
