...
export class MaterialTransactionService {
  ...
  private static async addWithDb(
    db: Awaited<ReturnType<typeof getDb>>,
    params: {
      materialId: number;
      quantity: number;
  ...
  static async spend(params: {
    ...
    return await withTransaction(async (db) => {
      return await MaterialTransactionService.spendWithDb(db, {
        materialId,
        quantity,
        reason,
        orderId,
  ...
  static async add(params: {
    ...
    return await withTransaction(async (db) => {
      return await MaterialTransactionService.addWithDb(db, params);
    });
  }
  ...
  static async checkAvailability(materialId: number, requiredQuantity: number): Promise<{
    ...
    // Считаем активные резервы. expires_at пишется как ISO (…T…Z) — сравниваем с ISO,
    // а не с sqlite datetime('now') (пробел вместо T), иначе same-day expiry ломается.
  ...
  static async bulkSpend(
    ...
    return await withTransaction(async (db) => {
      ...
      for (const item of items) {
        const result = await MaterialTransactionService.spendWithDb(db, {
  ...
  static async bulkAdd(
    ...
    return await withTransaction(async (db) => {
      const results: Array<{ materialId: number; oldQuantity: number; newQuantity: number }> = [];
      for (const item of items) {
        const result = await MaterialTransactionService.addWithDb(db, {
          materialId: item.materialId,
          quantity: item.quantity,
          reason: item.reason,
          userId
