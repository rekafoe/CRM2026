...
export class UserOrderPageService {
  ...
  static async getOrCreateUserOrderPage(userId: number, userName: string, date: string): Promise<UserOrderPage> {
    try {
      ...
      logger.info(`Created user order page for ${userName} on ${date}`);
      return this.mapRowToPage(newPage);
    } catch (error) {
      logger.error('Error creating user order page', error);
      throw error;
    }
  }
  ...
  static async getUserOrderPage(userId: number, date: string): Promise<UserOrderPage | null> {
    try {
      ...
      logger.error('Error getting user order page', error);
      return null;
    }
  }
  ...
  static async getUserOrderPages(userId: number, daysBack: number = 14): Promise<UserOrderPage[]> {
    try {
      ...
      logger.error('Error getting user order pages', error);
      return [];
    }
  }
  ...
  static async getAllOrderPages(daysBack: number = 90): Promise<UserOrderPage[]> {
    try {
      ...
      logger.error('Error getting all order pages', error);
      return [];
    }
  }
  ...
  static async assignOrderToPage(request: AssignOrderRequest): Promise<UserOrderPageOrder | null> {
    try {
      return await withTransaction(async (db) => {
        ...
        logger.info('Assigned order to page', { orderId: request.orderId, pageId: request.pageId });
        return this.mapRowToOrder(newAssignment);
      });
    } catch (error) {
      logger.error('Error assigning order to page', { request, error });
      throw error;
    }
  }
  ...
  static async getPageOrders(pageId: number): Promise<UserOrderPageOrder[]> {
    try {
      ...
      logger.error('Error getting page orders', error);
      return [];
    }
  }
  ...
  static async getPageChangesSinceLastUpdate(pageId: number, lastUpdate: string): Promise<{
    ...
    try {
      ...
      logger.error('Error getting page changes', error);
      return {
        newOrders: [],
        updatedOrders: [],
        completedOrders: [],
  ...
  static async updatePageStats(pageId: number): Promise<void> {
    try {
      ...
      logger.error('Error updating page stats', error);
    }
  }
  ...
  static async createTables(): Promise<void> {
    try {
      ...
      logger.info('User order pages tables created');
    } catch (error) {
      logger.error('Error creating user order pages tables', error);
      throw error;
    }
  }
